...
try
{
	...
	ResultSet rs1=st.executeQuery("select * from user inner join cart where cart.email='"+email+"' and cart.status='bill'");
	while(rs1.next())
	{
		
%>

<h3>Online_shopping Bill</h3>
<hr>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs2=st.executeQuery("select * from cart inner join product where cart.id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs2.next())
  {
	  sno=sno+1;
  %>
